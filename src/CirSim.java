...
//    Vector setupList;
    ...
    CirSim(Circuit a) {
	super("Electronics Visualization");
	applet = a;
	useFrame = false;
    }
	...
	Menu circuitsMenu = new Menu("Cts");
	if (useFrame)
	    mb.add(circuitsMenu);
	else
	...
	inputMenu.add(getClassCheckItem("Add  Voltage", "VarRailElm"));
...
//	setupList = new Vector();
	...
	if (printableCheckItem.getState()) {
	    ...
	    CircuitElm.whiteColor = Color.black;
	    CircuitElm.lightGrayColor = Color.green;
...
//		 ImportExportDialog.Action.EXPORT);
	...
	try {
	    URL url = new URL(getCodeBase() + "txtfiles/" + str);
	    ByteArrayOutputStream ba = readUrlData(url);
	    readSetup(ba.toByteArray(), ba.size(), false);
	} catch (Exception e1) {
	    try {
		URL url = getClass().getClassLoader().getResource("txtfiles/" + str);
		ByteArrayOutputStream ba = readUrlData(url);
		readSetup(ba.toByteArray(), ba.size(), false);
	    } catch (Exception e) {
		e.printStackTrace();
